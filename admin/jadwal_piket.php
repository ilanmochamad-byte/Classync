...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && !isset($_POST['arsipkan_massal'])) {
    ...
    if (isset($_POST['id']) && !empty($_POST['id'])) { // Edit
        $id = $_POST['id'];
        $stmt = $conn->prepare("UPDATE jadwal_piket SET guru_id=?, hari=?, sesi=?, status_jadwal=? WHERE id=?");
        $stmt->bind_param("isssi", $guru_id, $hari, $sesi, $status_jadwal, $id);
    } else { // Tambah
        $stmt = $conn->prepare("INSERT INTO jadwal_piket (guru_id, hari, sesi, status_jadwal) VALUES (?, ?, ?, ?)");
        $stmt->bind_param("isss", $guru_id, $hari, $sesi, $status_jadwal);
    }
    
    if($stmt->execute()) {
        $pesan = "Jadwal piket berhasil disimpan.";
        $pesan_tipe = "success";
    } else {
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['arsipkan_massal'])) {
    $update_arsip = $conn->query("UPDATE jadwal_piket SET status_jadwal = 'Arsip' WHERE status_jadwal = 'Aktif'");
    if ($update_arsip) {
        $pesan = "Berhasil! Semua jadwal piket aktif telah dipindahkan ke Arsip.";
        $pesan_tipe = "success";
        $_GET['status_jadwal'] = 'Arsip';
    } else {
        $pesan = "Gagal mengarsipkan jadwal piket.";
        $pesan_tipe = "danger";
    }
}
...
$sql = "SELECT jp.*, g.nama_guru FROM jadwal_piket jp JOIN guru g ON jp.guru_id = g.id WHERE jp.status_jadwal = ? ORDER BY g.nama_guru, FIELD(jp.hari, 'Senin', 'Selasa', 'Rabu', 'Kamis', 'Jumat', 'Sabtu', 'Minggu')";
...
<div class="d-flex justify-content-between align-items-center mb-4">
    <h2 class="fw-bold text-dark m-0"><i class="bi bi-shield-check text-primary me-2"></i> Manajemen Jadwal Piket</h2>
    <div class="d-flex gap-2">
        <form method="POST" action="" onsubmit="return confirm('PENTING: Tindakan ini akan mengarsipkan seluruh jadwal Piket Aktif saat ini untuk pergantian tahun pelajaran. Lanjutkan?')">
        ...
        <button class="btn btn-primary rounded-pill shadow-sm" data-bs-toggle="modal" data-bs-target="#jadwalModal">
            <i class="bi bi-plus-circle"></i> Tambah Piket
        </button>
    </div>
</div>
...
<div class="card shadow-sm border-0 rounded-4 mb-4">
    <div class="card-body p-3">
        <form method="GET" class="row g-3 align-items-center">
            <div class="col-md-6">
                <label for="status_jadwal" class="form-label fw-semibold small text-muted mb-1">Pilih Status Jadwal Piket yang Ditampilkan:</label>
            ...
            <table class="table table-hover align-middle mb-0">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="text-secondary"><?php echo htmlspecialchars($jadwal['sesi']); ?></td>
                        <td class="text-center pe-4">
                            <button class="btn btn-sm btn-outline-secondary rounded-circle me-1 btn-edit" 
                                    ...
                                    data-sesi="<?php echo htmlspecialchars($jadwal['sesi']); ?>"
                                    data-status_jadwal="<?php echo $jadwal['status_jadwal']; ?>"
                                    data-bs-toggle="modal" data-bs-target="#jadwalModal">
                                <i class="bi bi-pencil"></i>
                            </button>
                            <a href="jadwal_piket.php?hapus=<?php echo $jadwal['id']; ?>" class="btn btn-sm btn-outline-danger rounded-circle" onclick="return confirm('Yakin ingin menghapus jadwal ini?')">
                                <i class="bi bi-trash"></i>
                            </a>
                        </td>
                    </tr>
                    <?php endwhile; else: ?>
                        <tr><td colspan="5" class="text-center text-muted py-5"><i class="bi bi-calendar-x fs-1 d-block opacity-50 mb-2"></i> Belum ada jadwal yang ditemukan</td></tr>
                    <?php endif; ?>
                </tbody>
            </table>
        </div>
  ...
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content border-0 shadow rounded-4">
      <div class="modal-header border-0 pb-0">
        <h5 class="modal-title fw-bold text-primary" id="jadwalModalLabel"><i class="bi bi-pencil-square me-2"></i> Form Jadwal Piket</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
      </div>
      <form method="POST" action="jadwal_piket.php">
          <div class="modal-body">
              <input type="hidden" name="id" id="jadwal-id">
              <input type="hidden" name="status_jadwal" id="jadwal-status_jadwal" value="Aktif">
              
              <div class="mb-3">
                  <label for="guru_id" class="form-label fw-semibold text-muted">Guru <span class="text-danger">*</span></label>
              ...
              <div class="mb-3">
                  <label for="sesi" class="form-label fw-semibold text-muted">Sesi <span class="text-danger">*</span></label>
                  <input type="text" class="form-control bg-light" name="sesi" id="jadwal-sesi" placeholder="Contoh: Pagi, Siang" required>
              </div>
          </div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    jadwalModal.addEventListener('show.bs.modal', function (event) {
        ...
        const fields = ['id', 'guru_id', 'hari', 'sesi', 'status_jadwal'];

        if (button.classList.contains('btn-edit')) {
            modalTitle.innerHTML = '<i class="bi bi-pencil-square me-2"></i> Edit Jadwal Piket';
            fields.forEach(field => {
                const input = document.getElementById(`jadwal-${field}`);
                if(input) input.value = button.getAttribute(`data-${field}`);
            });
        } else {
            modalTitle.innerHTML = '<i class="bi bi-plus-circle me-2"></i> Tambah Jadwal Piket';
             fields.forEach(field => {
                const input = document.getElementById(`jadwal-${field}`);
