    ...
    <div class="col-md-8">
        <div class="card shadow">
            <div class="card-header bg-success text-white">
                <h4 class="mb-0">Form Absensi Piket</h4>
            </div>
            <div class="card-body">
                ...
                <form action="proses_absen.php" method="POST">
                    ...
                    <div class="d-grid">
                        <button type="submit" name="submit" id="tombol-absen" class="btn btn-success btn-lg" disabled>Konfirmasi Kehadiran Piket</button>
                    </div>
                </form>
            </div>
        </div>
...
document.getElementById('guru_id').addEventListener('change', function() {
    ...
    if (guruId) {
        ...
        fetch(`proses_absen.php?action=cek_jadwal&tipe=piket&guru_id=${guruId}`)
            .then(response => response.json())
            .then(data => {
                if (data.status === 'sukses') {
                    infoJadwalDiv.innerHTML = `
                        <h5>Jadwal Ditemukan:</h5>
                        <p><strong>Sesi Piket:</strong> ${data.jadwal.sesi}</p>
                    `;
