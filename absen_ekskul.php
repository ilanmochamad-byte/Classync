    ...
    <div class="col-md-8">
        <div class="card shadow">
            <div class="card-header bg-warning text-dark">
                <h4 class="mb-0">Form Absensi Ekstrakurikuler</h4>
            </div>
            <div class="card-body">
                ...
                <form action="proses_absen.php" method="POST">
                    ...
                    <div class="d-grid">
                        <button type="submit" name="submit" id="tombol-absen" class="btn btn-warning btn-lg" disabled>Konfirmasi Kehadiran Ekskul</button>
                    </div>
                </form>
            </div>
        </div>
...
document.getElementById('guru_id').addEventListener('change', function() {
    ...
    if (guruId) {
        ...
        fetch(`proses_absen.php?action=cek_jadwal&tipe=ekskul&guru_id=${guruId}`)
            .then(response => response.json())
            .then(data => {
                if (data.status === 'sukses') {
                    infoJadwalDiv.innerHTML = `
                        <h5>Jadwal Ditemukan:</h5>
                        <p><strong>Nama Ekskul:</strong> ${data.jadwal.nama_ekskul}</p>
