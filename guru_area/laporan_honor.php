...
if ($filter_bulan < 1 || $filter_bulan > 12 || $filter_tahun < 2000) {
    ...
    echo json_encode([
        'status' => 'error',
        'message' => 'Parameter bulan dan tahun wajib diisi dan harus berupa angka yang valid.'
    ]);
    exit;
}
...
try {
    ...
    $response = [
        ...
        'rincian' => [
            'pendapatan' => [
                ...
                'honor_mengajar'  => [
                    'total' => $rincian['honor_mengajar'],
                    'jp'    => $rincian['total_jp']
                ],
                'honor_piket'     => [
                    'total'  => $rincian['honor_piket'],
                    'jumlah' => $rincian['jumlah_piket']
                ],
                'honor_ekskul'    => [
                    'total'  => $rincian['honor_ekskul'],
                    'jumlah' => $rincian['jumlah_ekskul']
                ],
                'honor_bk'        => [
                    'total'  => $rincian['honor_bk'],
                    'jumlah' => $rincian['jumlah_bk']
                ],
    ...
    echo json_encode([
        'status' => 'error',
        'message' => 'Terjadi kesalahan saat membuat laporan honor.',
        'debug' => $e->getMessage()
    ]);
} finally {
